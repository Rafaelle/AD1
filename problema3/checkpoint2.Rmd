...
# trocando valores Na por 0 
  ...
  # emendas_ano_estado <- detalhes_emendas %>%
  #   group_by(UF_PROPONENTE,funcao.imputada, ANO_PROP, MES_PROP) %>%
  #   summarise(VALOR_REPASSE_PROPOSTA_EMENDA = sum(VALOR_REPASSE_PROPOSTA_EMENDA)) %>%
  #   group_by(funcao.imputada) %>%
  #   arrange(ANO_PROP,MES_PROP, UF_PROPONENTE) 
  #   
...
emendas_ano_log$ANO_PROP <- emendas_ano$ANO_PROP


emendas_parlamentar_log$Defesa.Nacional[ is.infinite(emendas_parlamentar_log$Defesa.Nacional) ] <- 0
